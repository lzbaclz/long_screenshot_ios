...
enum Pattern: String, CaseIterable {
    case texture, text, chat, table, sections, cards
    case periodic, gradient

    static let stable: [Pattern] = [.texture, .text, .chat, .table, .sections, .cards]
}
...
struct FixtureStep {
    enum Input {
        case document, changedScene, changedGeometry
    }
    let offset: Int
        ...
        for (patternIndex, pattern) in Pattern.stable.enumerated() {
            for depth in [5, 10, 20] {
                for variant in 0..<5 {
                    ...
                    let step = [73, 61, 87, 79, 69][variant]
                    ...
                    result.append(Fixture(
                        ...
                        top: variant == 2 || variant == 4 ? 17 : 0,
                        bottom: variant == 2 || variant == 4 ? 23 : 0,
