...
当上下忽略比例都为零时，管线通过多个中央区域和重放匹配确认用于内部接缝的区域，并在首次输出前冻结 `effectiveConfiguration`。输出保留最上端原始顶部与最下端原始底部各一次，向两端扩展时原子替换对应边缘，避免精确边界不明时静默裁掉正文。区域暂时不明确会等待更多证据；只有用户指定的非零手动范围会直接裁去对应内容。屏数上限按照有效正文高度计算，自动模式总高度还包含首尾边缘。
