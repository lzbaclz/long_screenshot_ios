...
- 本机采集诊断，包括帧计数、处理耗时、采集阶段及停止原因；诊断不包含识别出的文字或来源应用名称，也不会自动发送给开发者。起点候选图片在本机暂存，成功拼接后清理；中断后可作为明确标注的单屏恢复。
...
Local capture diagnostics include frame counts, processing time, capture stages and stop reasons. They contain no recognized screen text or source app names and are not automatically sent to the developer. A provisional starting image is stored locally until stitching is committed; after an interruption it may be recovered as a clearly labeled single-screen image.
