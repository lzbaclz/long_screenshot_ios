...
2026-09-15 更新：用户明确要求将 TestFlight 最新的 0.1.5（6）提交正式审核。本轮已补齐 Apple 要求的元数据与截图，17:45（Europe/London）实际提交成功，状态为“等待审核”。[正式提交记录](0.1.5-app-store.md) 记录具体构建、资料和验证边界；下列历史验收门槛仍按实际证据维护，未完成项不自动勾选。
...
| 发布时间方式与回退构建 | — |
| 审核人 / 发布负责人 / 决定日期 | — |
| 结论：继续验证 / 限定范围 / 准予提交 | **已按用户 2026-09-15 指令提交 0.1.5（6）正式审核，等待 Apple 审核；未完成的真机及用户研究门槛保持原状态** |
