...
**当前版本：0.1.4（构建 5）· 内部 TestFlight 测试中**
...
修复同色聊天页面向上翻时，状态栏和导航栏重复进入长图的问题。自动匹配先保护两帧共享的固定时间、图标与标题结构，候选边界和起拼重放都必须通过检查；壁纸前景区域按明确来源豁免。原始首尾保留一次，已捕捉正文按独立像素真值核对。

“捕捉详情”新增匹配方式、上下区域、原始帧高和固定结构保护范围。旧记录缺少数值时显示“未记录”，便于后续依据实际信息排查。

0.1.4（5）于 **2026-09-15** 开放内部 TestFlight，现有测试员可更新。更新后需重新开始捕捉验证；旧图不会自动重新拼接。完整说明见 [发布与验收记录](docs/release/0.1.4-testflight.md) 和 [开发流程](docs/validation/beta-5-acceptance.md)。
...
以下为 **0.1.4 发布时的本地验证记录**，不是每次打开 README 时重新运行的结果：

| 验证类别 | 结果 | 证明的范围 |
| --- | --- | --- |
| Swift 核心回归 | 76 / 76 通过 | 同色固定栏、覆盖层变化、候选来源及既有双向／壁纸／边界回归 |
| 合成真值基准 | 217 / 217 通过，共 8,776 帧 | 程序生成画面与独立真值的算法校验 |
| iOS 原生与像素验收 | 81 / 81 通过，0 跳过 | 完整固定栏／正文 PNG 像素、诊断、存储、暂停与额度等路径 |
| 模拟器滚动截图注入 | 6 / 6 路线通过，0 跳过 | 106 帧、94 次实际拖动，固定标题仅一处、消息与首尾像素核对 |
| 保存与分享回归 | 1 / 1 通过 | PNG、JPEG 保存及系统分享 |

完整结果、历史失败与验收口径见 [0.1.4 发布记录](docs/release/0.1.4-testflight.md)。上述结果不代表 iPhone 真机跨应用捕捉的成功率、内存、耗电或兼容性。
